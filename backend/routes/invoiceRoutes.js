...
// Roles allowed to manage invoices
const invoiceEditors = requireRole(['Admin', 'Manager', 'Sales']);
const invoiceAdmins = requireRole(['Admin', 'Manager']);
...
router.post('/', invoiceEditors, invoiceController.create);

// GET /api/v1/invoices/:id - Get single invoice with items and payments
router.get('/:id', invoiceController.getById);

// PATCH /api/v1/invoices/:id - Update invoice
router.patch('/:id', invoiceAdmins, invoiceController.update);

// DELETE /api/v1/invoices/:id - Delete invoice (unpaid only)
router.delete('/:id', invoiceAdmins, invoiceController.delete);

// POST /api/v1/invoices/:id/items - Add item to invoice
router.post('/:id/items', invoiceEditors, invoiceController.addItem);

// PATCH /api/v1/invoices/:id/items/:itemId - Update item price
router.patch('/:id/items/:itemId', invoiceAdmins, invoiceController.updateItemPrice);

// DELETE /api/v1/invoices/:id/items/:itemId - Remove item from invoice
router.delete('/:id/items/:itemId', invoiceAdmins, invoiceController.removeItem);

// POST /api/v1/invoices/:id/items/:itemId/void - Void item on paid invoice
router.post('/:id/items/:itemId/void', invoiceAdmins, invoiceController.voidItem);

// PATCH /api/v1/invoices/:id/discount - Update invoice-level discount
router.patch('/:id/discount', invoiceEditors, invoiceController.updateInvoiceDiscount);

// POST /api/v1/invoices/:id/payments - Receive payment (legacy endpoint)
router.post('/:id/payments', invoiceEditors, invoiceController.receivePayment);
...
router.post('/:id/transactions', invoiceEditors, invoiceController.createTransaction);

// POST /api/v1/invoices/:id/transactions/:txId/void - Void a transaction
router.post('/:id/transactions/:txId/void', invoiceAdmins, invoiceController.voidTransaction);

// POST /api/v1/invoices/:id/cancel - Cancel invoice (restore inventory)
router.post('/:id/cancel', invoiceAdmins, invoiceController.cancel);
